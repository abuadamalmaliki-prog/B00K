...
class Engine:
    ...
    def run(s, rows, htf_rows, pip, chart_sec):
        ...
        for i in range(n):
            ...
            if cd and ready:
                if htfOkL and poiL and trigL and rsi < 70 and scL >= P["minScore"] and kzGateOk and (trade is None or trade["dir"] != 1):
                    ok, rp, tps = plan(1)
                    longSig = ok
                if htfOkS and poiS and trigS and rsi > 30 and scS >= P["minScore"] and kzGateOk and (trade is None or trade["dir"] != -1):
                    ok2, rp2, tps2 = plan(-1)
                    shortSig = ok2
            if longSig and shortSig:  # both: keep the higher score, long on tie
                if scS > scL: longSig = False
                else: shortSig = False
            if longSig or shortSig:
                ...
                trade = dict(dir=dr, bar=i, entry=c, risk=rp * pip, sl=c - dr * rp * pip,
                             ...
                             time=T[i], score=scL if dr == 1 else scS, riskPips=rp, exit=None, fac=list(fL if dr == 1 else fS), trig=(engL, pinL, mssL) if dr == 1 else (engS, pinS, mssS), htfb=htfb)
                lastSig = i
                signals.append(i)
